...
void printGraph(const Graph &graph, const std::string& fileName, int n){
 ...
 std::cout<<"hi"<<numVertices;
    for(int source =0; source < numVertices ; source++){
       
        const auto& neighbours = graph.findNeighbours(source);
...
int main()
{
    ...
    std::unordered_map<int, std::vector<Graph::WeightingEdges>> graph = makeSparseUDWG(n, graphObj);
       printGraph(graphObj,"sparse25.txt", int n);
